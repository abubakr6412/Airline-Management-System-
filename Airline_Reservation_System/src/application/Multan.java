...
public class Multan implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		for (int i = 0; i < ARS.getInstance().Flight_list.size(); i++) {
			if (tempStatic.flightID == ARS.getInstance().Flight_list.get(i).getFlightID()
					&& ARS.getInstance().Flight_list.get(i).getDestination().equals("Multan")) {
	...
	public void Okaymessage(MouseEvent event) throws IOException {
		...
		bid = bid + 1;
		ARS.getInstance().AddBooking(bid, tempStatic.LoginID, tempStatic.seat, tempStatic.flightID);		   
